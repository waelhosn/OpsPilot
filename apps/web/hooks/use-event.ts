...
export const useEventCalendarStore = create<EventCalendarState>()(
  persist(
    (set, get) => ({
      ...
      setFirstDayOfWeek: () => set({ firstDayOfWeek: 1 }),
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        const state = persistedState as
          ...
          | undefined;
        const persistedViewSettings = state?.viewSettings;

        return {
          currentView: state?.currentView ?? CalendarViewType.DAY,
          viewMode: state?.viewMode ?? ViewModeType.CALENDAR,
          timeFormat: state?.timeFormat ?? TimeFormatType.HOUR_24,
          locale: state?.locale ?? 'en-US',
          firstDayOfWeek: 1,
          daysCount: state?.daysCount ?? 7,
          viewSettings: {
            day: {
              ...DEFAULT_VIEW_CONFIGS.day,
              ...(persistedViewSettings?.day ?? {}),
            },
            days: {
              ...DEFAULT_VIEW_CONFIGS.days,
              ...(persistedViewSettings?.days ?? {}),
              enableTimeBlockClick: true,
            },
            week: {
              ...DEFAULT_VIEW_CONFIGS.week,
              ...(persistedViewSettings?.week ?? {}),
              enableTimeBlockClick: true,
            },
            month: {
              ...DEFAULT_VIEW_CONFIGS.month,
              ...(persistedViewSettings?.month ?? {}),
            },
            year: {
              ...DEFAULT_VIEW_CONFIGS.year,
              ...(persistedViewSettings?.year ?? {}),
            },
          },
        };
      },
